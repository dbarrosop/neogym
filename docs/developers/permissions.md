...
### `nutrition_log_entries` — pattern **C (owned day + optional visible food + optional same-day group/provenance)**

Select/delete walk `nutritionDay.user_id eq self`. Insert checks the owned day, an explicit nullable food branch (`_or [food_id is null, food._or [user_id eq self, is_public eq true]]`), an explicit nullable group branch (`_or [nutrition_log_meal_id is null, nutritionLogMeal.nutritionDay.user_id eq self]`), and an explicit nullable direct-plan-food branch (`_or [nutrition_plan_food_id is null, nutritionPlanFood.nutritionPlan.user_id eq self]`). The database then enforces the source-specific shape: default `source = 'food'` rows require `food_id` and get trusted snapshots copied from `foods`; `source = 'ad_hoc'` rows require no `food_id`, no group, no plan-food provenance, and complete user-supplied snapshot fields. The composite FK rejects a group/day mismatch, and the plan-food provenance trigger rejects grouped plan-food rows or mismatched `food_id`. Users can set `slot_time` for standalone logged-food time on insert. Updates can correct `grams`, `position`, `slot_time`, and snapshot fields; a trigger rejects snapshot edits on food-backed rows while allowing them for ad-hoc rows. `source`, `food_id`, `nutrition_plan_food_id`, `nutrition_day_id`, and `nutrition_log_meal_id` are not user-updatable. Snapshot columns, `source`, and provenance columns are selectable so clients can compute historical totals and display provenance.
...
- **`nutrition_log_entries.source`, `nutrition_log_entries.food_id`, `nutrition_log_entries.nutrition_plan_food_id`, `nutrition_log_entries.nutrition_day_id`, and `nutrition_log_entries.nutrition_log_meal_id` after insert** — not in the user-role update allowlist. Source and provenance are insert-time facts. Snapshot columns are update-allowlisted only because ad-hoc rows need editable snapshots; the database trigger still rejects snapshot/name changes for food-backed rows.
