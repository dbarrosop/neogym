...
verifier clearing on all callback outcomes. Both `neogym://verify` and
`neogym-dev://verify` are allowed by `auth.redirections.allowedUrls` in
`backend/nhost/nhost.toml` and the production overlay; restart the local Nhost
stack after redirect config edits because the CLI does not hot-reload
