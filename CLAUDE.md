...
- **Production deploy** is a TanStack Start **SPA** build (shell prerendered, everything else CSR after hydration) served as static assets from **Cloudflare Workers Static Assets** via `wrangler`. Rationale: the app is auth-gated and used mostly as an installed PWA — SSR can't render personalized content (session lives in `localStorage`), and the SW caches the shell anyway, so paying per-request Worker cost for SSR buys nothing and complicates offline. SPA mode is enabled in `vite.config.ts` via `tanstackStart({ spa: { enabled: true, prerender: { outputPath: "/index" } } })`, which prerenders the root shell to `.output/public/index.html` at build time. The `frontend/wrangler.jsonc` is **assets-only** (no `main` field, no worker code) — just an `[assets]` binding pointing at `./.output/public` with `not_found_handling: "single-page-application"` so any unknown path falls through to the SPA shell for client-side routing. The PWA service worker handles offline navigations via `workbox.navigateFallback: "/index.html"` (denylisting `/api/*` and `/_*`). Deploy with `bun run deploy` (or `make deploy-prod`) — runs the build then `wrangler deploy`. `.wrangler/` and `.dev.vars` are gitignored and excluded from Biome. If you ever need server logic (API routes, server-rendered HTML for SEO on a public page), switch the nitro plugin to `preset: "cloudflare_module"` and add `main` back to `wrangler.jsonc`, but think twice — see the rationale above.
