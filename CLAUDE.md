...
Current devshell note: `bun` is available from Nix, but `rover` may not be. If `bun run codegen` fails at `codegen:graphql-schema` with `rover: command not found`, do not install random global tooling; either add rover to the Nix/toolchain explicitly in a separate tooling change, or use the checked-in frontend `graphql` dependency to run an equivalent introspection + `printSchema(buildClientSchema(...))` step, then run `bun run codegen:graphql`.
...
- **Nutrition GraphQL/logging shape**: meal logging should use one nested `insertNutritionLogMeal` with child `nutritionLogEntries`, and each child must explicitly include the same `nutritionDayId` as the parent group. Hasura nested inserts populate `nutritionLogMealId` but not the direct day FK used by permissions/composite same-day enforcement. Log time is user-selected and defaults to now: planned-meal logging may keep `nutritionPlanMealId` as provenance, but must not hardcode the logged `slotTime` to the template slot. Standalone food entries store their own `slotTime`; grouped entries display the parent logged meal's `slotTime`. Daily nutrition totals should be computed from logged `snapshot*Per100g` columns, not live `foods`.
